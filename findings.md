...
- `PhotoLibrary` is implemented. It loads/saves `photo_library` KV items, compresses large images before upload, previews with `object-contain`, submits `点线构图Lite` to `https://www.uiofield.top/meya/push`, and receives `guide_line_lite_result` over `wss://www.uiofield.top/meya/ws`.
...
- May 4 history made photo library submit Lite requests by HTTP and consume `guide_line_lite_result` results over WebSocket.
